...
: Node("image_detector")
{
  ...
  // 固定数のボール設定を個別に読み込む方法を使用
  for (int i = 0; i < 10; i++) {  // 最大10個のボール設定を試行
    ...
    try {
      ...
      // パラメータが既に宣言されている場合はスキップ
      continue;
    } catch (const std::exception& e) {
      // その他のエラーの場合はログ出力してスキップ
      RCLCPP_DEBUG(this->get_logger(), "No ball config at index %d", i);
      continue;
    }
  }
